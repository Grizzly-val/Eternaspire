...
public class IronDagger extends Weapon {
    public IronDagger(){
        super("Iron Dagger", "A reliable weight for the pocket. It won't win you a duel, but it's perfect for when subtlety is more important than strength.", "cutscene_FirstTimeEquip_IronDagger", 15);
    }
    ...
    public void basicAttack(Challenger user, Entity opponent, int atk) {
        ...
        if(totalDmg > 0 && counter > 0){
            System.out.println("| " + user.getName() + " used the Iron Dagger to perfection to land a " + counter + " hit combo!");
